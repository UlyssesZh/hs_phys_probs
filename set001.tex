...
\mypara
大量 A 粒子从原点以高速 $v$ 沿 $z$ 轴正方向射出.
建立柱坐标系 $\left(z,\rho,\varphi\right)$.
在 $\rho$ 柱面上建造 B 粒子探测器.
求足够长的时间之后探测器上探测到的 B 粒子的 $z$ 坐标的分布.
注意: 该分布应当被归一化, 使其在 $\left(-\infty,+\infty\right)$ 上的积分为 $2$.
...
设出射方向的角度为 $\theta'$.
由于出射方向按立体角均匀分布, $\theta'$ 的分布为
\begin{equation}
	\frac{\mathrm dN}N=\sin\theta'\,\mathrm d\theta'.
\end{equation}
...
于是出射粒子的动量为
\begin{equation}
	p'=\sqrt{E'^2-m^2}=\sqrt{M^2/4-m^2}.
\end{equation}
利用 Lorentz 变换可得实验室系中出射粒子的动量
\begin{equation}
	p_x=\frac{p_x'+vE'}{\sqrt{1-v^2}}=
	\frac{\sqrt{\frac{M^2}4-m^2}\cos\theta'+v\frac M2}{\sqrt{1-v^2}},
\end{equation}
\begin{equation}
	p_y=p_y'=\sqrt{\frac{M^2}4-m^2}\sin\theta'.
\end{equation}
于是出射粒子的方向为
\begin{equation}
	\theta=\operatorname{atan2}\!\left(p_y,p_x\right)=g\!\left(\theta'\right),
\end{equation}
其中
\begin{equation}
	g\!\left(\theta'\right)=\operatorname{atan2}\!\left(\sqrt{1-v^2}\sin\theta',\frac{Mv}{\sqrt{M^2-4m^2}}+\cos\theta'\right).
	\label{eq:g}
\end{equation}
$g$ 显然应是 $[0,\pi]$ 上可导的单调函数.

于是可以得到 $\theta$ 的分布为
\begin{equation}
	\frac{\mathrm dN}N=\frac{\sin g^{-1}\!\left(\theta\right)}{g'\!\left(g^{-1}\!\left(\theta\right)\right)}\mathrm d\theta,
	\label{eq:theta分布}
\end{equation}
其中 $g^{-1}$ 是 $g$ 在 $\left[0,\pi\right]$ 上的反函数.

式 \ref{eq:g} 和式 \ref{eq:theta分布} 给出了答案.

\mypara
衰变发生的时刻 $t$ 的分布为
\begin{equation}
	\frac{\mathrm dN}N=\frac{\sqrt{1-v^2}}\tau\mathrm e^{-\frac{t\sqrt{1-v^2}}\tau}\mathrm dt.
\end{equation}
在 $t$ 时刻以 $\theta$ 角出射的 B 粒子被探测到时其 $z$ 坐标为
\begin{equation}
	z=vt+\rho\cot\theta.
\end{equation}
因此被探测到 $z$ 坐标在 $\left[z,z+\mathrm dz\right]$ 中的 B 粒子数为
\begin{equation}
	N\iint_{z\le vt+\rho\cot\theta\le z+\mathrm dz}
	\frac{\sqrt{1-v^2}}\tau\mathrm e^{-\frac{t\sqrt{1-v^2}}\tau}\mathrm dt
	\cdot
	\frac{\sin g^{-1}\!\left(\theta\right)}{g'\!\left(g^{-1}\!\left(\theta\right)\right)}\mathrm d\theta.
\end{equation}
从而 $z$ 的分布为
\begin{equation}
	\frac{\mathrm dN}N=h'\!\left(z\right)\mathrm dz,
\end{equation}
其中
\begin{equation}
	h\!\left(z\right)=\frac{\sqrt{1-v^2}}\tau
	\iint_{vt+\rho\cot\theta\le z}
	\frac{\sin g^{-1}\!\left(\theta\right)}{g'\!\left(g^{-1}\!\left(\theta\right)\right)}
	\mathrm e^{-\frac{t\sqrt{1-v^2}}\tau}\mathrm dt
	\mathrm d\theta.
\end{equation}
